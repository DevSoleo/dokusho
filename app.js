...
const User = mongoose.model('User', { username: String, time_bank: Number, status: Object, offers_end: Number })
...
io.on('connection', (socket) => {
	...
	socket.on('ask_for_create_user', (username) => {
		User.findOne({ username: username }, {}, function(err, arr){		
			if (arr == null) {
				User.create({ username: username, status: { connected: false, connection_timestamp: 0 }, offers_end: 0, time_bank: 0 }, () => {})
	...
	socket.on('ask_for_login_user', (username) => {
		User.findOne({ username: username }, {}, function(err, arr){		
			if (arr != null) {
				if (arr['status']['connected'] == false) {
					if (arr['time_bank'] > 0 || arr['offers_end'] > Date.now()) {
						User.updateOne({ username: username }, { status: { connected: true, connection_timestamp: Math.floor(new Date() / 1000)} }, {}, function() {})
	...
	socket.on('ask_for_logout_user', (username) => {
		User.findOne({ username: username }, {}, function(err, arr){			
			if (arr != null) {
				if (arr['status']['connected'] == true) {
					User.updateOne({ username: username }, { status: { connected: false, connection_timestamp: 0 } }, {}, function() {})
...
setInterval(function() {
    ...
    User.updateMany({ 'status.connected': true, time_bank: {$gte: 1000}, offers_end: { $lt: Date.now() } }, { $inc: { time_bank: -1000 } }, () => {})

	// Envoie de la liste des utilisateurs connectés aux clients
	User.find({ 'status.connected': true }, (err, arr) => {
